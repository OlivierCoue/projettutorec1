...
OperatorTable * IMPLEMENT(OperatorTable_loadFromFile)(const char * filename) {
    ...
    char name[OPERATORTABLE_MAXNAMESIZE]="";
    char password[OPERATORTABLE_MAXPASSWORDSIZE]="";
    OperatorTable * newOperatorTable;
    int recordCount, i;
    FILE * file = fopen(filename, "r+");

    newOperatorTable = OperatorTable_create();
    if (file == NULL)
        return NULL;
...
void IMPLEMENT(OperatorTable_saveToFile)(OperatorTable * table, const char * filename) {
    ...
    recordCount = OperatorTable_getRecordCount(table);
